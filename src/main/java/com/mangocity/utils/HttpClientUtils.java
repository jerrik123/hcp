...
public class HttpClientUtils {
	 ...
	 * @throws IOException
	 */
	public static String readContentFromGet(String url, Map<String, String> params) throws IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (i == 0) {
				buffer_URL.append("?").append(pa).append("=").append(URLEncoder.encode(va, "utf-8"));
			} else {
				buffer_URL.append("&").append(pa).append("=").append(URLEncoder.encode(va, "utf-8"));
			}
		}
	...
	private static HttpPost httpPost(String url, Object params){  
        if(url != null && params != null){
        	...
        	if(params instanceof Map){
                ...
                httpost.setEntity(new StringEntity(params.toString(), Charset.forName("UTF-8")));
                return httpost;
        	}
        }
        return null;
